...
let html = '<html><head><link href="styles.css" rel="stylesheet" type="text/css"></head><body><table><tr><th>Collection</th><th>Active bids</th><th>Expired bids</th><th>Total tried</th><th>Assets</th><th>To bid</th><th>Earliest bid</th><th>h since</th><th>Latest bid</th><th>h since</th></tr>'
...
async function report() {
  await Object.keys(c.collections).forEach(async colName => {
    ...
    add(`<tr><td>${colName}</td><td>${t.active}</td><td>${t.expired}</td><td>${t.total}</td><td>${assets.length}</td><td>${t.toBid}</td><td>${t.earliestBid}</td><td class="${diff1>BID_H?'green':'red'}">${diff1}</td><td>${t.latestBid}</td><td class="${diff2>BID_H?'green':'red'}">${diff2}</td></tr>`)
  })

  add(`<tr><td></td><td>${tt.active}</td><td>${tt.expired}</td><td>${tt.total}</td><td>${tt.assets}</td><td>${tt.toBid}</td><td></td><td></td><td></td><td></td></tr>`)
